...
public class DetailsAssets : MonoBehaviour
{
    ...
    public static void NewGeneratedObjects(GameObject slotPanel, GameObject slot1, GameObject item1, List<Basic_Information_model> emList)
    {
    ...
    public void GeneratedObjects(GameObject slotPanel, GameObject slot, GameObject item, List<Basic_Information_model> emList)
    {
    ...
    public void Objects(Basic_Information_model em, GameObject item, int i)
    {
        GameObject itemObject = Instantiate(item);
        itemObject.transform.SetParent(slots[i].transform);
        itemObject.transform.position = Vector2.zero;
        itemObject.name = em.id;
        ...
        TxtID.text = "编号：" + em.id;
        TxtName.text = "名称：" + em.AssetName;
        TxtSpecifications.text = "规格：" + em.DeviceType;
        TxtNumber.text = "数量：" + em.InstallDate;
        TxtUnit.text = "单位：" + em.DateAcceptance;
        TxtRemarks.text = "备注：" + em.WarrantyDate;
    }
